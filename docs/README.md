...
- We plan on using [Opencode](https://opencode.ai/) specifically as our primary tool for any AI features, as it provides multiple free online models (with local functionality too, see above) and allows for the creation of specialised agents which we can use for particular tasks. For this project, we may create a level generator agent with the context of how to generate YAML level files for the game.
