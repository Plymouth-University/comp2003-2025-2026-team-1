...
class LevelEditor:
    ...
    def setup_ui(self):
        ...
        palette_combo = ttk.Combobox(controls_frame, textvariable=self.palette_var, width=15, state="readonly")
        palette_combo['values'] = ['__ (empty)', '01', '02', '03', 'a1', 'b1', 'gm', 'floor', 'wall']
        palette_combo.pack(side=tk.LEFT, padx=5)
        palette_combo.bind('<<ComboboxSelected>>', self.on_palette_select)
    ...
    def update_grid_display(self):
        # Clear old images
        for img_id in self.cell_images.values():
        ...
        for row in range(self.grid_rows):
            for col in range(self.grid_cols):
                ...
                # Remove old text (we're using images now, Q2: colored rectangle only)
                if text_id:
    ...
    def on_palette_select(self, event):
        ...
        if value:
            code = value.split(' ')[0]  # Remove description
    ...
    def load_yaml(self, filename):
        try:
            ...
            # Merge with shared definitions if set
            if self.shared_defs_path.get() and os.path.exists(self.shared_defs_path.get()):
                shared_data, shared_error = load_yaml_file(self.shared_defs_path.get())
                if shared_data and not shared_error:
                    # Merge shared data first, then file data takes precedence
                    if 'objectDefinitions' in shared_data and shared_data['objectDefinitions']:
                        if 'objectDefinitions' in merged_data and merged_data['objectDefinitions']:
                            # Merge: shared first, then file's (file takes precedence)
                            merged_defs = shared_data['objectDefinitions'].copy()
                            merged_defs.update(merged_data['objectDefinitions'])
                            merged_data['objectDefinitions'] = merged_defs
                        else:
                            merged_data['objectDefinitions'] = shared_data['objectDefinitions']
                    if 'gridObjects' in shared_data and shared_data['gridObjects']:
                        if 'gridObjects' in merged_data and merged_data['gridObjects']:
                            merged_defs = shared_data['gridObjects'].copy()
                            merged_defs.update(merged_data['gridObjects'])
                            merged_data['gridObjects'] = merged_defs
                        else:
                            merged_data['gridObjects'] = shared_data['gridObjects']
    ...
    def write_yaml(self, filename):
        try:
            # Update full_yaml_data with current edits
            # Build grid string - match original format with sections
            grid_lines = []
            for i, row in enumerate(self.grid_data):
                # Pad row to grid_cols
                padded_row = row + ['__'] * (self.grid_cols - len(row))
                line = ','.join(padded_row) + ','
                grid_lines.append('  ' + line)
                
                # Add blank line between sections (every 8 rows as in example)
                if (i + 1) % 8 == 0 and (i + 1) < len(self.grid_data):
                    grid_lines.append('')
                    
            grid_str = '\n'.join(grid_lines)
            
            # Update the full YAML data
            if not self.full_yaml_data:
                self.full_yaml_data = {}
            
            # Update file properties
            creator = getattr(self, 'creator_name', 'Level Editor User')
            self.full_yaml_data['fileProperties'] = {'creatorName': creator}
            
            # Update scene name
            scene = getattr(self, 'scene_name', 'OriginalWorld')
            self.full_yaml_data['sceneName'] = scene
            
            # Update grid - use LiteralString to force | style
            self.full_yaml_data['grid'] = LiteralString(grid_str)
            
            # Update gridObjects (only save non-empty)
            if self.grid_objects:
                non_empty = {k: v for k, v in self.grid_objects.items() if v}
                if non_empty:
                    self.full_yaml_data['gridObjects'] = non_empty
                elif 'gridObjects' in self.full_yaml_data:
                    del self.full_yaml_data['gridObjects']
                    
            # Update objectDefinitions
            if self.object_definitions:
                self.full_yaml_data['objectDefinitions'] = self.object_definitions
                    
            # Remove internal tracking keys
            if '_original_includes' in self.full_yaml_data:
                del self.full_yaml_data['_original_includes']
            
            # Write to file
            with open(filename, 'w') as f:
                yaml.dump(self.full_yaml_data, f, default_flow_style=False, sort_keys=False, allow_unicode=True)
    ...
    def display_cell_image(self, row, col, filepath, bg_color):
        """Display an image centered in the cell"""
        ...
        try:
            ...
            # Check cache
            cache_key = (filepath, int(self.cell_size))
            if cache_key in self.image_cache:
                photo = self.image_cache[cache_key]
            else:
                # Load and resize image
                img = Image.open(filepath)
                # Resize to fit cell (leave small margin)
                size = int(self.cell_size * 0.85)
